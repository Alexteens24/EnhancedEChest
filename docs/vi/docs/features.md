...
<DocCard icon="BarChart2" title="Thống Kê Sử Dụng" link="#usage-statistics" desc="Số liệu và báo cáo lỗi ẩn danh qua bStats và FastStats." />
...
EnhancedEchest sử dụng hai dịch vụ telemetry:

- [bStats](https://bstats.org/plugin/bukkit/EnhancedEchest/32142) thu thập thống kê sử dụng plugin ẩn danh. Có thể tắt bStats toàn cục trong `plugins/bStats/config.yml`.
- Các bản phát hành chính thức dùng FastStats để báo loại cơ sở dữ liệu và ngôn ngữ đã cấu hình, đồng thời thu thập báo cáo lỗi đã ẩn danh. Plugin không chủ ý đính kèm danh tính người chơi, nội dung rương hay thông tin đăng nhập cơ sở dữ liệu. FastStats không hoạt động trong bản build không chứa project token; hiện chưa có thiết lập phía server để tắt riêng dịch vụ này.
