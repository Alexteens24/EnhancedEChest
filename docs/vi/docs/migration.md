...
EnhancedEchest có thể nhập dữ liệu rương Ender sẵn có vào kho lưu trữ của riêng nó để không mất gì khi cài plugin. Hỗ trợ bốn nguồn: rương Ender **vanilla**, plugin **AxVaults**, plugin **PlayerVaultsX** và plugin **CustomEnderChest**.
...
EnhancedEchest có thể nhập rương Ender từ plugin [CustomEnderChest](https://modrinth.com/plugin/custom-ender-chest), bao gồm mọi vật phẩm cùng tên tùy chỉnh, lore và phù phép. Tính năng này đã được kiểm thử với **CustomEnderChest 2.1.2**.

Khác với AxVaults/PlayerVaultsX, CustomEnderChest chỉ cho mỗi người chơi **một** rương Ender duy nhất (kích thước theo quyền hạng cao nhất của họ), không có nhiều kho đánh số, nên nó được nhập vào **rương #1** của EnhancedEchest, giống mục tiêu của chuyển dữ liệu vanilla.

### Trước Khi Bắt Đầu

Việc nhập đọc trực tiếp các tệp người chơi của CustomEnderChest, nên cần lưu ý một điều:

- **CustomEnderChest phải được đặt sang lưu trữ YAML.** EnhancedEchest đọc các tệp theo từng người chơi trong `CustomEnderChest/playerdata/`, tệp này chỉ tồn tại khi `storage.type: yml` được đặt trong `CustomEnderChest/config.yml`. CustomEnderChest mặc định dùng cơ sở dữ liệu `h2` tích hợp sẵn thay vì vậy, nên hãy chuyển sang `yml` và khởi động lại server nguồn trước (giống thao tác chuyển đổi một lần mà chuyển dữ liệu AxVaults đã yêu cầu), rồi mới chuyển dữ liệu. Backend `mysql` cũng không được đọc.
...
Cả hai đều cần quyền `enhancedechest.admin.migrate`. Việc nhập chạy nền và báo số người chơi đã nhập khi hoàn tất.

::: tip An toàn khi chạy lại
Rương Ender của một người chơi sẽ **không bao giờ** được nhập đè lên rương #1 nếu nó đã có vật phẩm. Nếu người chơi đã dùng rương #1 của EnhancedEchest rồi, việc nhập sẽ được báo là bỏ qua và giữ nguyên, nên chạy lệnh hai lần cũng không nhân đôi hay ghi đè bất cứ thứ gì.
:::
